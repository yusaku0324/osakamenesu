...
function getApiBases(): string[] {
  ...
  const isLocalPublic = publicBase.includes('localhost') || publicBase.includes('127.0.0.1')
  return onServer && isLocalPublic ? [internalBase, publicBase] : [publicBase, internalBase]
}

async function fetchShop(id: string): Promise<ShopDetail> {
  for (const base of getApiBases()) {
    try {
      const r = await fetch(`${base}/api/v1/shops/${id}`, { cache: 'no-store' })
